...
- [x] 2026-08-15：将完整 `Integer#times { total += document.render.bytesize }` 接入统一 typed caller hot region：由完整 Register IR 数据流证明 captured `Document`/`total`，首轮缓存严格 Prawn wrapper/Renderer method target，后续直接进入 Renderer object-layout/mutation epoch region；builtin `Integer#+`/`String#bytesize`、方法/常量代际、Prawn class extension、Document identity 和所有 Renderer guard 失效时从未执行的迭代 side-exit 到原 Ruby block，int64 溢出先提交 BigInt 再回放 suffix。支持无参数及未使用单参数 block，重定义、overflow、mutation、分页、结构/冻结/删 ivar 与 digest 回归一致。单核低优先级：100,000 次完整脚本连续样本 `0.182s/0.182s`，既有基线 `0.717–0.738s`（约 `3.9–4.1x`）；20,000 次 Renderer region `0.140s` 对关闭 region `1.884s`（约 `13.5x`）。这是当前严格闭世界 workload 的稳定样本，不外推到任意 Prawn 文档。
- [x] 2026-08-15：继续收敛 `Renderer#render` 缓存命中尾段：按 mutation epoch、Renderer/State/Store class 缓存输入 guard；缓存命中后用对象布局/已有 map 的 typed bookkeeping setter 写回 `@page/@page_number/@offset/@xref_offset`，未知布局保留 generic side-exit。`freeze`、`singleton_class`、`remove_instance_variable` 也会失效缓存，避免布局变化漏检。单核低优先级样本：20,000 次 Renderer region 开启/关闭为 `0.114s/1.732s`（约 `15.2x`），100,000 次完整脚本为 `0.254s` 对既有基线 `0.668s`（约 `2.63x`；另一轮候选最佳 `0.207s`）；输出、digest、mutation、分页、重定义、overflow 与结构/冻结/删 ivar side-exit 保持一致。完整脚本剩余 profile 主要是 `Integer#times`/通用 VM 循环和每次返回 String 包装，不把局部样本外推为通用稳定 `3–10x`。
- [x] 2026-08-15：将完整 `PDF::Core::Renderer#render` 的稳定命中路径扩展为 typed layout 输出缓存：记录节点标量/字符串快照、Reference/page 绑定和 mutation epoch；Array/Hash/String/对象 ivar 语义变更会失效并回到原 typed writer/serializer，`@offset/@page/@page_number/@xref_offset` 等 render bookkeeping 不会误驱逐缓存。修正同一 key 下无输出缓存模板先 bind 的顺序问题，并补齐 native page-stack 直接写入的 epoch 标记。单核低优先级 A/B：100,000 次完整脚本候选 `0.224s`、既有对照 `0.638s`（约 `2.85x`），20,000 次 Renderer region 开启/关闭 `0.160s/1.795s`（约 `11.2x`）；输出 `133000000`、可变文档、6 组大图 SHA-256、overflow、重定义和分页 side-exit 均一致。当前完整脚本受 `Integer#times`/VM 循环与启动成本限制，仍不把 `2.85x` 外推为通用稳定 `3–10x`。
- [x] 2026-08-15：继续收敛完整 `PDF::Core::Renderer#render` 的 typed/object-layout 热区：为 Reference/Stream/FilterList 建立精确 inline slot，`Object` 维护布局字段代际；同图模板加入严格的节点边/Hash key/Array 元素/对象代际 fast-bind，Reference stream plan 也按代际复用，任一字段、键、边或可变字符串漂移立即回到完整 binder/serializer。新增同图复用、Hash 变更回退和布局外 `@offset` 不冲刷缓存的回归；单核低优先级 100,000 次输出仍为 `133000000`，前一已推送版本约 `0.71s`、本候选约 `0.64s`（局部约 `1.1x`），20,000 次 region 开启/关闭约 `0.273s/1.796s`（该特定 renderer 子路径约 `6.6x`）。默认 PDF、大图 SHA-256、三页、overflow、重定义和变更后再次 render 均与 fallback 一致；`pkg/core` 全过，`pkg/vm` 仍只有既有 3 项无关失败。测试保持单核串行，CPU Tctl 约 `43.6°C`、NVMe 最高读数约 `56.9°C`；仍不宣称通用稳定 `3–10x`，更大收益需要跨文档构建的统一 JIT/typed loop。
- [x] 2026-08-15：继续收敛完整 `PDF::Core::Renderer#render` 的统一 typed/object-layout hot region：模板缓存 Ruby class pointer，binder 使用 epoch/bound-count 代际守卫，预编译程序合并相邻 literal，并将稳定 ASCII 字符串与静态 Float 降为直接字面量；完整 preflight 后 trailer、array/hash/reference writer 走 trusted path，任何 Hash/key、类、UTF-8、浮点漂移或 Ruby 可见状态变化仍 side-exit。新增 ASCII/Float drift、typed trailer 回归；单核低负载同一 PDF 100,000 次输出保持 `133000000`，f8d19d3 基线约 `0.97s`、候选约 `0.74–0.77s`（约 `1.3x`），动态三页 5000 次输出保持 `8933358`，region 开启/关闭约 `0.79s/1.39s`（约 `1.76x`）。测试期间 CPU 保持单核、温度约 `46°C`；完整 Go gate 仍只复现 TODO 已记录的 3 个无关 VM 失败，未宣称稳定 `3–10x`。
- [x] 2026-08-15：继续收紧 Renderer typed layout hot region：复用模板绑定/Reference/page scratch，预编译 array/hash 的线性 write program，将 nil/bool/int/float/symbol/string/reference 叶子降为带节点索引的 typed op，并对普通 map-backed Reference ivar 使用 layout-aware 读取；float 走无 `fmt` 的定长数值格式化，所有节点/代际/stream/cache guard 和 Ruby side-exit 保持不变。新增 compiled writer 与数值格式语义测试；重建后二页/大图/overflow/redefine smoke 输出分别保持 `1334`、`2000`、`34478 94a28cb432147ced9ae56c367a8f122534550087ab3cdc19f7f27c87db7922c6`、`9223372036854778476`、`2676`。单核低负载 20,000 次同图 render 约 `0.336s`，关闭整个 region 约 `1.8s`；关闭更高层 Prawn AOT/lifecycle 后动态三页 5000 次为 `0.907s` 对 `1.281s`（约 `1.41x`），仍不宣称稳定 `3–10x`，剩余成本主要是对象布局 guard、Hash/ivar map 读取和通用 VM 文档构建。
