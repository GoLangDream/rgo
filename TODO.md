...
- [ ] 最新发布基线（2026-07-25）：Go 全仓门禁 `safe_go_test.sh ./... -count=1` 全绿；RubySpec 全树真实串行刷新为 `3809 files`，其中 `3256 pass / 526 guard-zero / 25 nonzero / 2 timeout`，共执行 `31639 examples / 53 failures`，报告为 `reports/spec-status/ruby-spec-full.csv`。当前主要失败簇为 Marshal 10、Thread 7+1 timeout、Delegate 6、Module 4、Kernel 5、ObjectSpace 4、Net::HTTP 9，以及若干独立命令行/Regexp/RbConfig 缺口；旧的 `33510/0` 记录不再作为当前发布结论。
