...
- [x] 最新发布基线（2026-07-27）：Go 全仓门禁 `safe_go_test.sh ./... -count=1` 全绿；RubySpec 全树真实串行刷新为 `3809 files`，其中 `3283 pass / 526 guard-zero`，共执行 `31653 examples / 0 failures / 0 timeout`，报告为 `reports/spec-status/ruby-spec-full.csv`。本轮清零 Marshal、Thread、Delegate、Module、Kernel、ObjectSpace、Net::HTTP、命令行、Regexp、RbConfig、BigDecimal 等剩余缺口，并保留 guard-zero 与行为通过的边界。
